...
import { contactAutomationView, failuresView } from "../lib/automation-views.js";
  ...
  try {
    ...
    if (view === "failures") {
      const sinceHours = Math.min(
        Math.max(parseInt(url.searchParams.get("sinceHours") || String(DEFAULT_FAILURE_WINDOW_HOURS), 10) || DEFAULT_FAILURE_WINDOW_HOURS, 1),
        24 * 90,
      );
    ...
    return new Response(JSON.stringify({ error: "Unknown view (use contact or failures)" }), { status: 400, headers });
  } catch (err) {
