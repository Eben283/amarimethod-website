...
export const INITIAL_IN_PERSON_WORKFLOW = defineWorkflow({
  ...
  nodes: [
    ...
    { id: "reschedule-confirmation", label: "Send updated booking confirmation", at: "reschedule", action: { type: "email", template: "reschedule-confirmation" }, skipIfPast: false, message: { audience: "client", channel: "email", from: "Amari Method <eben@amarimethod.com>", subject: "Your appointment time has been updated", body: "Hi {{firstName}},\n\nYour session with Garrett has been updated:\n{{calendarName}}\n{{appointmentFull}}\n{{location}}\n\nWear something comfortable you can move in. That's all you need.\n\n{{appointmentLinks}}\n\nWe look forward to seeing you.\nThe Amari Method Team" } },
...
export function initialInPersonNode(template, workflow = INITIAL_IN_PERSON_WORKFLOW) {
  ...
  if (!node && template === "reschedule-confirmation") {
    return INITIAL_IN_PERSON_WORKFLOW.nodes.find((candidate) => candidate.action.template === template) || null;
  }
  return node || null;
}
