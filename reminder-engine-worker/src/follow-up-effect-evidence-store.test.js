...
describe("durable effect candidate against actual promoted-v2 SQLite", () => {
  ...
  it("keeps the three inert evidence modules and offline planner isolated from production and schema routes", () => {
    ...
    function visit(dir) {
      for (const entry of readdirSync(new URL(dir, root), { withFileTypes: true })) {
        ...
        if (!/\.(?:js|mjs|cjs|ts|tsx|json|toml|sql)$/.test(path) || /(?:\.test\.|\.spec\.|\/fixtures\/)/.test(path)
          || ["functions/lib/follow-up-effect-evidence-store.js", "functions/lib/follow-up-evidence-composition.js", "functions/lib/follow-up-consumer-retention-store.js", "scripts/follow-up-evidence-install-plan.mjs"].includes(path)) continue;
        const source = readFileSync(new URL(path, root), "utf8");
        if (/follow-up-effect-evidence-store|follow-up-evidence-composition|follow-up-consumer-retention-store|follow-up-evidence-install-plan|reliability-effect-evidence\.candidate|reliability-consumer-retention\.candidate/.test(source)) violations.push(path);
      }
    }
