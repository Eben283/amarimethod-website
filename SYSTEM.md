...
|------|------|-------|-------|
| Slot policy (duration/interval/buffer) | Native catalog | `functions/lib/booking-slot-policy.js` | Assessment + Follow-up both use the reviewed duration/buffer/hourly policy. Partner Initial stays 60 minutes. `applyHourPackPreference` + look-busy govern public/portal slots. Doc: `decision_booking_slot_model.md` |
...
| Staff Calendar workspace | Native UI + governed provider command | `staff/src/pages/TodayPage.tsx`, `staff/src/components/CalendarRegistry.tsx`, `staff/src/components/ManageAppointmentSheet.tsx`, `functions/api/staff-data.js`, `functions/api/staff-calendars.js`, `functions/api/staff-appointments.js`, `functions/lib/staff-calendar-catalog.js`, `functions/lib/staff-appointment-manage.js`, `functions/lib/staff-owned-contact-identity.js`, `functions/lib/staff-owned-appointment-store.js`, `functions/lib/staff-owned-appointment-schedule.js`, `functions/lib/staff-calendar-provider-ghl.js`, `db/appointment-commands-migration.sql`, `crm-mirror-worker/migrations/0019_owned_appointment_authority.sql`, `crm-mirror-worker/src/owned-appointments.js`, `crm-mirror-worker/src/owned-appointment-schedule.js` | Primary `/staff/calendar` destination: operational day/week/month schedule, service timing/cutover registry, and one Appointment Manager for new scheduling, rescheduling, and cancellation. Internal availability is every collision-free 15-minute start inside Garrett’s governed workday; public slot thinning/clustering is deliberately not applied. Signed actor, owned person/appointment identity, server-owned service/calendar identity, calendar policy, conflicts, idempotency, creation/replacement checkpoints, compensation, and provider readback are enforced server-side. New appointment search submits the owned person ID. The temporary GHL crosswalk is resolved again at the server-side calendar-adapter boundary; older provider-backed Staff references are resolved back to the owned person before a command is claimed or recorded, so durable command evidence never adopts the provider contact ID as its owner. GHL request translation is isolated in the temporary compatibility adapter rather than the Staff API or command domain. The source-only owned-authority path preserves existing dependent IDs, captures a canonical collision-checked Partner Initial appointment before propagation, leases one executor, checkpoints or compensates the exact temporary GHL link, completes only after provider readback, and records retry/rejection/manual-review evidence. Provider webhooks can reconcile or flag owned truth but cannot overwrite it. Week/month source now reads owned appointment rows and visibly reports authoritative/propagating/mirrored/degraded counts without provider fallback; detailed day contact/payment/ledger hydration remains explicitly provider-backed until moved coherently. No production schema or runtime switch exists until the separate install/release gates. Existing lifecycle owners remain unchanged. |
...
| GHL communication source archive | `crm-mirror-worker/src/sync.js`, `crm-mirror-worker/src/providers.js`, `crm-mirror-worker/migrations/0035_ghl_communication_source_archive.sql` | Exact provider JSON is preserved before Staff projection. Separate scheduled email and non-email export streams advance through bounded historical windows without writing to GHL; projection, media, reconciliation, and exit proof remain separate gates. |
...
| Owned public quiz intake bridge | `functions/api/send-to-ghl.js`, `functions/lib/owned-quiz-intake-forward.js`, `crm-mirror-worker/src/owned-quiz-intake.js` | The public Pages boundary retains origin, Turnstile, rate-limit and idempotency control, reads the existing GHL contact before applying the current Quiz tag, then maps only the normalized quiz contract through an authenticated same-account `CRM_MIRROR` service binding. A private header carries conclusive `present`/`absent` provider-history evidence without changing the strict body contract; malformed or unavailable provider evidence fails before capture or provider writes. The first qualifying quiz creates one lifetime Flow 1 handoff. Provider history, an imported Quiz tag, any earlier owned Quiz submission, or an earlier dispatch suppresses re-entry while still retaining the new intake and refreshing Quiz facts. Missing history headers preserve the previous behavior for staggered deployment compatibility. Owned acknowledgement must precede the temporary GHL compatibility write. Source mode permits capture, but a separate `OWNED_QUIZ_BRIDGE_RELEASE=approved` flag, private binding and Worker auth are all mandatory; any missing control fails before GHL. GHL remains the compatibility intake and live nurture owner until retirement is separately approved. |
