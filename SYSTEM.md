...
| Staff Calendar workspace | Native UI + governed provider command | `staff/src/pages/TodayPage.tsx`, `staff/src/components/CalendarRegistry.tsx`, `staff/src/components/ManageAppointmentSheet.tsx`, `functions/api/staff-calendars.js`, `functions/api/staff-appointments.js`, `functions/lib/staff-calendar-catalog.js`, `functions/lib/staff-appointment-manage.js`, `db/appointment-commands-migration.sql` | Primary `/staff/calendar` destination: operational day/week/month schedule, service timing/cutover registry, and Staff cancel/reschedule. Internal availability is every collision-free 15-minute start inside Garrett’s governed workday; public slot thinning/clustering is deliberately not applied. Signed actor, person/appointment ownership, calendar policy, conflicts, idempotency, replacement checkpoint, compensation, and provider readback are enforced server-side. Existing reminder/cancellation lifecycle owners remain unchanged. |
...
| Calendar / day-of | `TodayPage`, `CalendarRegistry`, `AppointmentCard`, `ManageAppointmentSheet`, `staff-appointments` | Staff schedule, service-definition visibility, and the governed cancel/reschedule command. The same control is reachable from the Member Record. It never uses public free-slot presentation rules and never lets the browser author calendar/status identity. |
