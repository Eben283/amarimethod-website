...
| Owned public quiz intake bridge | `functions/api/send-to-ghl.js`, `functions/lib/owned-quiz-intake-forward.js`, `crm-mirror-worker/src/owned-quiz-intake.js` | The public Pages boundary retains origin, Turnstile, rate-limit and idempotency control, then maps only the normalized quiz contract through an authenticated same-account `CRM_MIRROR` service binding. Owned acknowledgement must precede the temporary GHL compatibility write and the old provider-contact nurture event is removed. The first qualifying quiz creates one lifetime Flow 1 handoff; later quizzes retain immutable intake evidence and refresh quiz facts without restarting nurture. Source mode permits capture, but a separate `OWNED_QUIZ_BRIDGE_RELEASE=approved` flag, private binding and Worker auth are all mandatory; any missing control fails before GHL. GHL remains the compatibility intake and live nurture owner until retirement is separately approved. |
