...
| Lifecycle reliability spine | `functions/lib/reliability-contract.js`, `functions/lib/reliability-store.js`, `reminder-engine-worker/src/follow-up-reliability.js`, `functions/api/staff-automations.js`, `staff/src/pages/AutomationRegistryPage.tsx` | Follow-Up-only authoritative source receipts, lifecycle instances, obligations, exceptions, and truthful Staff evidence. Runtime capture is fail-closed and inert unless the exact feature flag and provenance bindings are present; it never replaces the reminder sender or genericizes workflow execution. |
