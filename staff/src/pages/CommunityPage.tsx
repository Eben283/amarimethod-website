...
export default function CommunityPage() {
  ...
  return <main className="community-page">
    ...
    {error ? <section className="community-empty community-empty--error"><p>{error}</p><button type="button" onClick={() => void load()}>Try again</button></section> : loading ? <section className="community-empty"><RefreshCw className="animate-spin" aria-hidden="true" /><p>Loading field relationships…</p></section> : partners.length === 0 ? <section className="community-empty"><Building2 aria-hidden="true" /><h2>No field relationships yet</h2><p>After the first visit, log the business through Chief of Staff and it will appear here.</p></section> : <div className="community-lanes"><section className="community-lane community-lane--next"><header><div><p>Do next</p><span>Scheduled outreach</span></div><strong>{nextMoves.length}</strong></header>{nextMoves.length ? nextMoves.map((partner) => <RelationshipCard key={partner.id} partner={partner} kind="next" onOpen={openPartner} />) : <p className="community-lane__empty">Nothing scheduled yet. Set the next touch on an active relationship.</p>}</section><section className="community-lane"><header><div><p>Active relationships</p><span>Keep the thread alive</span></div><strong>{active.length}</strong></header>{active.length ? active.map((partner) => <RelationshipCard key={partner.id} partner={partner} kind="active" onOpen={openPartner} />) : <p className="community-lane__empty">No relationships are waiting for a next step.</p>}</section><section className="community-lane community-lane--events"><header><div><p>Events & workshops</p><span>Prepare the opportunity</span></div><strong>{events.length}</strong></header>{events.length ? events.map((partner) => <RelationshipCard key={partner.id} partner={partner} kind="event" onOpen={openPartner} />) : <p className="community-lane__empty">Events and workshop conversations will collect here.</p>}</section></div>}
