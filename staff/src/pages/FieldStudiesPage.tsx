...
type FormErrors = Partial<Record<'firstSessionCompleted' | 'fieldStudyKey' | 'firstName' | 'lastName' | 'phone' | 'email' | 'afterSessionOnePain', string>>;
type ParticipantForm = { firstSessionCompleted: boolean | null; fieldStudyKey: FieldStudyKey | ''; firstName: string; lastName: string; phone: string; email: string; paperDate: string; canUseFirstName: boolean; afterSessionOnePain: number | null; participantQuote: string };
...
function blankParticipant(): ParticipantForm { return { firstSessionCompleted: null, fieldStudyKey: '', firstName: '', lastName: '', phone: '', email: '', paperDate: localDate(), canUseFirstName: false, afterSessionOnePain: null, participantQuote: '' }; }
...
export default function FieldStudiesPage() {
  ...
  function confirmDiscard() { return !((view === 'participant' && formDirty) || (view === 'baseline' && baselineDirty)) || window.confirm('You have unsaved work. Leave without saving?'); }
  function navigate(next: View) { if (!confirmDiscard()) return; setView(next); if (next === 'participant') setRecord(null); }
  function returnHome() { if (confirmDiscard()) goTo('/'); }
  ...
  return <main className="field-study-shell"><section className="field-study-ipad">
    <header className="fs-topbar"><div className="fs-topbar-brand"><button type="button" onClick={() => navigate('participant')}><i />AMARI METHOD</button><button type="button" className="fs-home-link" onClick={returnHome}><House size={14} /> Home</button></div><div><span><LockKeyhole size={13} /> Staff view</span><time>{new Intl.DateTimeFormat('en-US', { month: 'short', day: 'numeric' }).format(new Date())}</time></div></header>
    ...
    {view === 'participant' && <section className="fs-intake"><form onSubmit={saveParticipant} className="fs-form" noValidate><div className="fs-heading"><div><p>{form.firstSessionCompleted === false ? 'BEFORE SESSION ONE' : 'AFTER SESSION ONE'}</p><h1>Finish participant record</h1><span>Fields marked * are required before saving.</span></div></div><fieldset className={errors.firstSessionCompleted ? 'fs-session-choice has-error' : 'fs-session-choice'}><legend>Did they complete their first session? *</legend><div><button type="button" className={form.firstSessionCompleted === true ? 'selected' : ''} aria-pressed={form.firstSessionCompleted === true} onClick={() => changeForm('firstSessionCompleted', true)}>Yes, session 1 is complete</button><button type="button" className={form.firstSessionCompleted === false ? 'selected' : ''} aria-pressed={form.firstSessionCompleted === false} onClick={() => changeForm('firstSessionCompleted', false)}>No, book session 1</button></div>{errors.firstSessionCompleted && <small role="alert"><AlertCircle size={12} /> {errors.firstSessionCompleted}</small>}</fieldset><div className="fs-study-row"><Field label="Study *" error={errors.fieldStudyKey}><select name="study" value={form.fieldStudyKey} onChange={(e) => changeForm('fieldStudyKey', e.target.value as FieldStudyKey | '')} aria-invalid={Boolean(errors.fieldStudyKey)}><option value="">Choose the paper panel…</option>{(Object.entries(FIELD_STUDIES) as Array<[FieldStudyKey, typeof FIELD_STUDIES[FieldStudyKey]]>).map(([key, study]) => <option key={key} value={key}>{study.label} — {study.note}</option>)}</select>{selectedStudy && <small>{selectedStudy.note}</small>}</Field><div className="fs-paper-id"><b>Paper form label</b><strong>{paperLabel}</strong><small>First name · study · today — matching the sheet in hand.</small></div></div>{form.firstSessionCompleted && <><section className={errors.afterSessionOnePain ? 'fs-score has-error' : 'fs-score'}><p>After session one *</p><div><strong>Where is their pain now?</strong><small>0 = none · 10 = most intense</small>{errors.afterSessionOnePain && <><AlertCircle size={12} /> {errors.afterSessionOnePain}</>}</div><Scale label="After-session pain score" value={form.afterSessionOnePain} onChange={(v) => changeForm('afterSessionOnePain', v)} /></section><section className="fs-quote"><div><p>IN THEIR WORDS</p><strong>Participant quote Optional</strong><small>Capture it as closely as possible.</small></div><textarea name="participant-quote" value={form.participantQuote} onChange={(e) => changeForm('participantQuote', e.target.value)} maxLength={500} rows={3} placeholder="What feels different? What feels easier? What surprised you?" /></section></>}<div className="fs-divider" /><section className="fs-details"><p>Participant details</p><div className="fs-two"><Field label="First name *" error={errors.firstName}><input name="given-name" autoComplete="given-name" value={form.firstName} onChange={(e) => changeForm('firstName', e.target.value)} aria-invalid={Boolean(errors.firstName)} /></Field><Field label="Last name *" error={errors.lastName}><input name="family-name" autoComplete="family-name" value={form.lastName} onChange={(e) => changeForm('lastName', e.target.value)} aria-invalid={Boolean(errors.lastName)} /></Field></div><div className="fs-two"><Field label="Mobile *" error={errors.phone}><input name="tel" autoComplete="tel" inputMode="tel" value={form.phone} onChange={(e) => changeForm('phone', formatPhone(e.target.value))} aria-invalid={Boolean(errors.phone)} placeholder="(415) 555-0123" /></Field><Field label="Email *" error={errors.email}><input name="email" autoComplete="email" type="email" spellCheck={false} value={form.email} onChange={(e) => changeForm('email', e.target.value.trimStart())} aria-invalid={Boolean(errors.email)} placeholder="name@example.com" /></Field></div><label className="fs-consent"><input type="checkbox" checked={form.canUseFirstName} onChange={(e) => changeForm('canUseFirstName', e.target.checked)} /> <span>May we use their first name in study notes? Optional</span></label></section><div className="fs-footer"><small>{form.firstSessionCompleted === false ? 'Save their details, then choose a time for session 1.' : 'The 6 baseline answers stay on paper. Enter them once the participant has left.'}</small><button type="submit" disabled={saving}>{saving ? <><Loader2 size={17} className="animate-spin" /> Saving…</> : <>{form.firstSessionCompleted === false ? 'Save and book session 1' : 'Save participant'} <ArrowRight size={17} /></>}</button></div>{record && <section className="fs-success" aria-live="polite"><div><Check size={20} /><span><b>{record.paperId} saved</b><small>{record.firstSessionCompleted ? 'Participant record, Study Name, field-table tag & session 1 are recorded.' : 'Participant record and Study Name are saved. Session 1 is not recorded yet.'}</small>{record.bookingStatus === 'loaded' && <small className="fs-booked-summary">{record.bookedSessions?.length ? `Study sessions: ${bookedDates(record.bookedSessions)}` : 'No study sessions booked yet.'}</small>}</span></div><div className="fs-success-action"><span>{calendarOpened ? `Amari calendar opened — book ${record.firstSessionCompleted ? 'sessions 2 and 3' : 'session 1'}, then close it to refresh these dates.` : 'Next: book the next session.'}</span><button type="button" onClick={() => { setCalendarOpened(true); setCalendarOpen(true); }}>Open Amari calendar <ExternalLink size={14} /></button></div></section>}</form></section>}
...
function CalendarModal({ record, onClose }: { record: FieldStudyParticipant; onClose: () => void | Promise<void> }) {
  ...
  const firstSessionNumber = firstSessionCompleted ? 2 : 1;
  const sessionNumber = firstSessionNumber + booked.length;
  const sessionsToBook = firstSessionCompleted ? 2 : 3;
  ...
  return <div className="fs-calendar" role="dialog" aria-modal="true" aria-label="Book study sessions">
    ...
    <section className="fs-native-calendar">
      ...
      <div className="fs-cal-intro"><p>STUDY SESSIONS · SAN FRANCISCO</p><h2>Book the next session.</h2><span>Choose a 15-minute study session with Garrett for {record.firstName}.</span></div>
      <div className="fs-cal-progress" aria-label="Booking progress"><span className="active"><i>01</i> Pick a time</span><b /><span className={booked.length >= 1 ? 'active' : ''}><i>02</i> Session {firstSessionNumber}</span><b /><span className={booked.length >= 2 ? 'active' : ''}><i>03</i> Session {firstSessionNumber + 1}</span>{sessionsToBook === 3 && <><b /><span className={booked.length >= 3 ? 'active' : ''}><i>04</i> Session 3</span></>}</div>
