...
//
// If the client has already signed (KV attestation exists), render a
// read-only view showing the policies as agreed-to plus the stored
// signature. Re-signing is not exposed in the UI — once on file, it stays.
...
export default function CheckInPage() {
  ...
  const [attestation, setAttestation] = useState<StaffAttestation | null>(null);
    ...
    <div className="min-h-screen bg-amari-light-sand">
      <header className="sticky top-0 bg-white border-b border-amari-border z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-base font-semibold text-amari-charcoal">
            {alreadySigned ? 'Policies signed' : 'Check in'}
          </h1>
        </div>
      </header>
...
function SignedView({
  ...
  attestation: Extract<StaffAttestation, { found: true }>;
  client: ContactDetail | null;
}) {
  const signedDate = new Date(attestation.signedAt);
  const formattedDate = signedDate.toLocaleString(undefined, {
    dateStyle: 'long',
    timeStyle: 'short',
  });
  ...
  return (
    <>
      <section className="staff-card">
        <p className="text-xs font-semibold text-amari-text-muted uppercase tracking-wider mb-1">
          ...
          {client && (client.firstName || client.lastName) && (
            <span> by {client.firstName} {client.lastName}</span>
          )}
        </p>
      ...
      <section className="staff-card">
        <h3 className="text-sm font-semibold text-amari-charcoal mb-3">Agreed to</h3>
      ...
      <section className="staff-card">
        <h3 className="text-sm font-semibold text-amari-charcoal mb-3">Signature on file</h3>

        <div className="mb-4">
          <span className="text-xs font-medium text-amari-text-muted uppercase tracking-wider block mb-1">
            Full name
          </span>
          <p className="text-amari-charcoal">{attestation.typedName}</p>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="border border-amari-border rounded-lg bg-white p-2 inline-block max-w-full">
            <img
              src={attestation.signatureImage}
              alt={`Signature of ${attestation.typedName}`}
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-xs text-amari-text-muted">
          <div>
            <dt className="font-medium uppercase tracking-wider">Signed at</dt>
            <dd className="text-amari-charcoal">{formattedDate}</dd>
          </div>
          <div>
            <dt className="font-medium uppercase tracking-wider">Agreement version</dt>
            <dd className="text-amari-charcoal break-all">{attestation.agreementVersion}</dd>
          </div>
        </dl>
      </section>
    </>
