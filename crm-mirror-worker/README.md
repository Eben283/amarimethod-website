...
- `POST /contacts/quiz-intake` — defense-in-depth Worker-authenticated normalized quiz source capture. It validates a strict allowlist, preserves exact owned identity, and returns only the owned contact ID plus a payload digest; no provider, nurture, or delivery call occurs.
- `GET /quiz-intake/readiness` — authenticated aggregate-only source count, latest capture time, and expired-retention count; no contact or quiz answers.
